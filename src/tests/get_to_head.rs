...
mod for_handlers {
    // NOTE: when adding test cases here make sure to add them for
    // `for_handlers` as well.
    use super::*;
...
mod for_services {
    ...
    // `for_handlers` as well.
    use super::*;
    use crate::service::{get, head, post};
    ...
    async fn head_explicitly_defined_last() {
        let app = route(
            "/",
            get((|| async { "hi from GET" }).into_service()).head(
                (|| async {
                    let mut headers = HeaderMap::new();
                    headers.insert("x-some-header", "foobar".parse().unwrap());
                    (headers, "you shouldn't see this")
    ...
    async fn head_explicitly_defined_first() {
        let app = route(
            ...
            .get((|| async { "hi from GET" }).into_service()),
        );
    ...
    async fn head_explicitly_defined_middle() {
        let app = route(
            "/",
            post((|| async { "hi from POST" }).into_service())
                .head(
                    (|| async {
                        let mut headers = HeaderMap::new();
                        headers.insert("x-some-header", "foobar".parse().unwrap());
